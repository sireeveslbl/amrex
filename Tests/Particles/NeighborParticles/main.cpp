...
#pragma omp parallel
        ...
        for (MyParIter pti(*this, lev, MFItInfo().SetDynamic(false)); pti.isValid(); ++pti) {
            ...
            for (int j = 0; j < size; ++j) {
                amrex::AllPrintToFile("nl") << neighbor_list[index][j] << " ";
            }
            amrex::AllPrintToFile("nl") << std::endl;
        }
    }
...
private:
    ...
    inline virtual bool check_pair(const ParticleType& p1, const ParticleType& p2) final {
        return (std::abs(p1.pos(0) - p2.pos(0)) < 0.046875) and 
               (std::abs(p1.pos(1) - p2.pos(1)) < 0.046875);
    }
};
